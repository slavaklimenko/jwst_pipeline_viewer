...
class additional_fit_data():
    def __init__(self, group_dq=None, jumps_value=None, intercept_val=None):
        self.group_dq = group_dq
        self.jumps_value = jumps_value
        self.intercept_val = intercept_val

#def ols_ramp_fit_single(ramp_data, buffsize, save_opt, readnoise_2d, gain_2d, weighting):
def custom_ramp_fit(ramp_data, buffsize, save_opt, readnoise_2d, gain_2d,max_cores):
    ...
    readnoise_2d *= gain_2d  # convert to readnoise in electrons/time

    #for i in range(n_int):
    #    for j in range(ngroups):
    #       ramp_data.data[i,j] *= gain_2d  # convert DN/time to electrons/time
    #       ramp_data.err[i, j] *= gain_2d
    if ngroups == 1:
    ...
    fit_slopes_ans = ramp_fit_slopes(ramp_data, gain_2d, readnoise_2d, save_opt)

    plt.subplots()
    plt.hist(fit_slopes_ans[6].slope_2d[0],bins=np.linspace(-4,5,10),log=True)
    # in format of
    # (0) max_seg,
    # (1) gdq_cube_shape,
    # (2) f_max_seg,
...
def ramp_fit_slopes(ramp_data, gain_2d, readnoise_2d, save_opt):
    """
    ...
    #max_seg = ngroups-1     # max number of segments fit in all integrations
    max_seg = 1
    f_max_seg = 1           # actual maximum number of segments over all groups and segments


    dq_int, num_seg_per_int, sat_0th_group_int = utils.alloc_arrays_1(n_int, imshape) #allocate info arrays for each integration for each pixel
    opt_res = utils.OptRes(n_int, imshape, max_seg, ngroups, save_opt)  #allocate "optional results"
    opt_res.allcrmag = np.zeros((n_int,ngroups-1, nrows, ncols))
    opt_res.groupdq = groupdq.copy()
    rlo,rhi = 0, cubeshape[1]

    pixeldq = inpixeldq.copy()
    pixeldq = utils.reset_bad_gain(ramp_data, pixeldq, gain_2d)  # Flag bad pixels in gain

    # create a class for groupdq_array, jumps and intercept
    #ramp_data.add_data = additional_fit_data()
    #ramp_data.add_data.group_dq = ramp_data.groupdq.copy()
    #ramp_data.add_data.jumps_value = np.zeros((n_int, ngroups-1, nrows, ncols))
    #ramp_data.add_data.intercept_val = np.zeros_like((n_int, ngroups-1, nrows, ncols))

    # Loop over data integrations:
    for num_int in range(0, n_int):
        ramp_data.current_integ = num_int
        data_sect = data[num_int, :, :, :].copy()

        # Get appropriate sections
        gdq_sect = groupdq[num_int, :, :, :].copy()
        rn_sect = readnoise_2d[:, :]
        # gain_sect = gain_2d[:, :]
        ...
        data_sect *= gain_2d  # convert DN/time to electrons/time
        #err *= gain_2d

        t_dq_cube, inv_var, opt_res, f_max_seg, num_seg =  calc_slope(data_sect, gdq_sect, frame_time,
            opt_res, save_opt, rn_sect, gain_2d,  max_seg, f_max_seg, ramp_data,num_int)

        if save_opt:
            # convert back to DN/s
            gain_1d  = np.reshape(gain_2d,nrows*ncols)
            #opt_res.slope_seg /= gain_2d
            opt_res.slope_2d /= gain_1d
            opt_res.sigslope_2d /= gain_1d
            opt_res.allcrmag[num_int,:] /=gain_2d
            #opt_res.cr_mag_seg /= gain_2d
            opt_res.interc_2d/=gain_1d
            opt_res.siginterc_2d/=gain_1d
            # PRELIMINARY: CHECK IT
            opt_res.inv_var_2d*=gain_1d**2
            inv_var*=gain_1d**2

        # Populate 3D num_seg { integ, y, x } with 2D num_seg for this data section (y,x) and integration (num_int) # always = 1
        sect_shape = data_sect.shape[-2:]
        num_seg_per_int[num_int, rlo:rhi, :] = num_seg.reshape(sect_shape)

        # Populate integ-spec slice which is set 1 if 0th group has SATURATED flag
        sat_0th_group_int[num_int, rlo:rhi, :][np.bitwise_and(gdq_sect[0, :, :], ramp_data.flags_saturated).astype(bool)] = 1

        # return gdq_sect, inv_var, opt_res, f_max_seg, num_seg
        pixeldq_sect = pixeldq[:, :].copy()
        dq_int[num_int, :, :] = utils.dq_compress_sect(ramp_data, num_int, t_dq_cube, pixeldq_sect).copy()

        #PRELIMINARY: save cr events
        #ramp_data.add_data.group_dq[num_int] = np.bitwise_or(ramp_data.groupdq[num_int],t_dq_cube)

        #ramp_data.add_data.jumps_value[num_int] = opt_res.alljumps
        #ramp_data.add_data.intercept_val[num_int] = opt_res.intercept
        ...
        opt_res.groupdq[num_int] = t_dq_cube
    ...
    # define med_rates over iterations
    med_rates = np.mean(opt_res.slope_seg[:,0,:,:],axis=0)
...
def ramp_fit_overall(
    ...
    warnings.resetwarnings()
    #slope_dataset2 = np.mean(opt_res.slope_seg[:,0,:,:],axis=0)
    ...
    if save_opt:
        ...
        for num_int in range(0, n_int):
            ...
            opt_res.ped_int[num_int, :, :] = \
                utils.calc_pedestal(ramp_data, num_int, slope_int, opt_res.firstf_int, dq_slice, nframes, groupgap, dropframes1)
    ...
    if save_opt:
        if 1:
            ...
            for ii_int in range(0, n_int):
                dq_cube = opt_res.groupdq[ii_int, :, :, :]
                dq_cr = np.bitwise_and(jump_det, dq_cube)
            ...
            for ii_int in range(0, n_int):
                dq_cube = opt_res.groupdq[ii_int, 1:, :, :]
                dq_cr = np.bitwise_and(jump_det, dq_cube)
                for i_rows in range(nrows):
                    for j_cols in range(ncols):
                        mask_jumps = np.where(dq_cr[:,i_rows,j_cols]>0)
                        if np.sum(mask_jumps)>0:
                            jumps = opt_res.allcrmag[ii_int,:,i_rows,j_cols][mask_jumps]
                            #print(mask_jumps.jumps)
                            #print(opt_res.allcrmag[ii_int,:,i_rows,j_cols])
                            for k in range(np.size(mask_jumps)):
                                cr_com[ii_int,k,i_rows,j_cols] = jumps[k]
            opt_res.cr_mag_seg = cr_com
            del dq_cube
            del dq_cr
        #gdq_cube = groupdq
        ...
        if 1:
            opt_info = (opt_res.slope_seg, opt_res.sigslope_seg, opt_res.var_p_seg,
                        opt_res.var_r_seg, opt_res.yint_seg, opt_res.sigyint_seg,
                        opt_res.ped_int, opt_res.groupdq, opt_res.cr_mag_seg)
    else:
        opt_info = None
    ...
    if 1:
        ...
        for ii_int in range(n_int):
            for gg_group in range(ngroups):
                final_pixeldq = np.bitwise_or(final_pixeldq,opt_res.groupdq[ii_int,gg_group])
        ...
        #final_pixeldq = np.bitwise_or(final_pixeldq,opt_res.group_dq
...
def calc_slope(data_sect, gdq_sect,  frame_time, opt_res, save_opt, rn_sect,gain_sect,
                i_max_seg, f_max_seg, ramp_data,n_int):
    # input: data_sect, gdq_sect, frame_time, opt_res, save_opt, rn_sect,  max_seg, f_max_seg, ramp_data
    """
    ...
    skip_gr_cr_neighboors = True
    skip_groups_above_cr = True
    ...
    readtimes = np.arange(1, ngroups+1)

    C_sect = fitramp.Covar(readtimes)
    diffs_sect = (im[1:] - im[:-1])/C_sect.delta_t[:, np.newaxis, np.newaxis]
    sig = rn_sect

    alljumps = np.zeros(diffs_sect.shape)
    alljumpsigs = np.zeros(diffs_sect.shape)
    slopes_sect = np.zeros(sig.shape)
    sig_slopes_sect = np.zeros(sig.shape)
    ped_sect = np.zeros(sig.shape)
    sig_ped_sect = np.zeros(sig.shape)
    inv_var = np.zeros(sig.shape)
    num_seg = np.zeros(sig.shape)

    if 1:
        for i_rows in range(nrows):
            diffs2use, countrates = fitramp.mask_jumps(diffs_sect[:,i_rows,:], C_sect, sig[i_rows], threshold_oneomit=20.25, threshold_twoomit=23.8)
            ct = countrates * (countrates > 0)

            mask_bad_pix = gdq_sect[1:, i_rows, :] != 0    #(np.bitwise_and(groupdq[:, 0, :, :], ramp_data.flags_jump_det))
            diffs2use[mask_bad_pix] = 0
            mask_jumps = (diffs2use == 0) * ~mask_bad_pix
            if not skip_groups_above_cr:
                result = fitramp.fit_ramps(diffs_sect[:, i_rows,:], C_sect, sig[i_rows], diffs2use=diffs2use, detect_jumps=True, countrateguess=ct)
                #mask_jumps = (diffs2use == 0) * ~mask_bad_pix
            else:
                 for i_cols in range(diffs2use.shape[1]):
                    i_gr = np.where(mask_jumps[:,i_cols] == True)[0]
                    if np.size(i_gr)>0:
                        for el in i_gr[i_gr>int(ngroups/3)]:
                            diffs2use[el:, i_cols] = 0
                            if i_cols>0 and i_cols<diffs2use.shape[1]-1 and skip_gr_cr_neighboors==True:
                                diffs2use[el:, i_cols-1] = 0
                                diffs2use[el:, i_cols +1] = 0

                 result = fitramp.fit_ramps(diffs_sect[:, i_rows, :], C_sect, sig[i_rows], diffs2use=diffs2use,
                                           detect_jumps=True, countrateguess=ct)

            diffs2use[mask_jumps] = ramp_data.flags_jump_det
            # PRELIMINARY - STARTS from 0 to last-1 (?)
            (gdq_sect[1:, i_rows, :])[mask_jumps] = np.bitwise_or((gdq_sect[1:, i_rows, :])[mask_jumps],ramp_data.flags_jump_det)

            alljumps[:,i_rows,:] = result.jumpval_oneomit
            alljumpsigs[:,i_rows,:] = result.jumpsig_oneomit
            slopes_sect[i_rows,:] = result.countrate
            sig_slopes_sect[i_rows, :] = result.uncert
            if 1:
                ped_sect[i_rows,:] = (im[0, i_rows, :] + im[1, i_rows, :]) / 2
                sig_ped_sect[i_rows, :] = np.zeros_like(ped_sect[i_rows,:]) + 1


            for j in range(len(diffs_sect)):
                indx = diffs2use[j] == ramp_data.flags_jump_det  # only need to redo these differences
                if np.sum(indx) == 0:
                    continue
                # each time we'll make sure that this difference isn't masked
                mask = diffs2use[:, indx] * 1
                mask[j] = 1
                result = fitramp.fit_ramps(diffs_sect[:, i_rows, indx], C_sect, sig[i_rows, indx], diffs2use=mask,
                                           detect_jumps=True, countrateguess=ct[indx])
                #print('moCR at ',i_rows, np.where(indx==True))
                #for kk,jj in enumerate(np.where(indx==True)[0]):
                #    print('slope',slopes_sect[i_rows,jj],' after:',result.countrate[kk] )

                # Overwrite the jump value if it was previously masked.
                alljumps[j,i_rows,indx] = result.jumpval_oneomit[j]
                alljumpsigs[j,i_rows, indx] = result.jumpsig_oneomit[j]

                if 0:
                    for i_cols in np.where(indx==True)[0]:
                        fig,ax = plt.subplots(2,1)
                        ax[0].plot(im[:,i_rows,i_cols],'o')
                        ax[0].plot(ped_sect[i_rows,i_cols] + slopes_sect[i_rows,i_cols]*readtimes)
                        ax[0].plot(ped_sect[i_rows, i_cols]+ alljumps[j,i_rows, i_cols]+ slopes_sect[i_rows, i_cols] * readtimes,'--')
                        ax[0].axvline(j)
                        ax[1].plot(diffs_sect[:,i_rows,i_cols],'o')
                        ax[1].axhline(slopes_sect[i_rows,i_cols])
                        ax[1].axvline(j)
    ...
    if 1:
        plt.subplots()
        plt.imshow(slopes_sect/gain_sect,vmin=0,vmax=2)
        #plt.show()

    if 1:

        slopes_sect /= ramp_data.group_time
        sig_slopes_sect /= ramp_data.group_time
        variance = sig_slopes_sect ** 2.

        g_pix = (variance > 0.)*(~np.isinf(variance))  # good pixels
        inv_var[g_pix] += 1.0 / variance[g_pix]
        slopes_sect[~g_pix] = 0.0
        # set number of segments to 1
        num_seg[g_pix] = 1

        # convert results to default form of 1d array
        # Create object to hold optional results
        opt_res.init_2d(npix, i_max_seg, save_opt)
        opt_res.slope_2d[0] = np.reshape(slopes_sect,npix)
        opt_res.sigslope_2d[0] =  np.reshape(sig_slopes_sect,npix)
        opt_res.inv_var_2d[0] = np.reshape(inv_var, npix)

        if save_opt:
            opt_res.allcrmag[n_int] =alljumps[:]
            opt_res.interc_2d[0] = np.reshape(ped_sect,npix)
            opt_res.siginterc_2d[0] = np.reshape(sig_ped_sect,npix)
            #opt_res.inv_var_2d = np.reshape(inv_var,npix)
            #opt_res.alljumps_2d = np.zeros_like(alljumps)
            #opt_res.alljumps_2d = alljumps

        inv_var = np.reshape(inv_var,npix)
        num_seg = np.reshape(num_seg,npix)
...
if __name__ == '__main__':
    print('Hi PyCharm')
    #miri_uncal_file = 'jw02155003001_05101_00001_mirifushort_uncal.fits'
    #input_dir = '/media/slava/14999070-ec17-4bcc-993d-c556030e9642/home/slava/science/data/JWST/ID2155/J0901/UNCALIB/jw02155003001_05101_00001_mirifushort'
    miri_uncal_file = 'jw02155002001_06101_00004_mirifushort_uncal.fits'
    input_dir = '/media/slava/14999070-ec17-4bcc-993d-c556030e9642/home/slava/science/data/JWST/ID2155/J0900/UNCAL/jw02155002001_06101_00004_mirifushort'
    ...
    exp1.drop_first_steps()
    exp1.last_step()
    exp1.slope_fitting_step(algorithm='CHI2',save_results=True)
    plt.subplots()
    plt.imshow(exp1.ramp_fit[0].data,vmin=0,vmax=2)
    plt.show()
