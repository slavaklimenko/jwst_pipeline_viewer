...
def read_psf(channel='1A_ch1-short_test.fits',source='custom_psf',overdist = False):
...
def model_img(params , cube_shape, psf_cube, spectrum_model, overdist=False, debug=False, get_qso_pos=False):
    ...
    if overdist==False:
        ...
        if 0:
            for i in range(cube_shape[0]): #cube_shape[0]
                model_convolved[i] = convolve2d(
                    model_cube[i],
                    psf_cube[i],
                    mode='same',
                    boundary='fill',
...
def plot_comparison(data, params,psf_cube,mask_fitting, spectrum_model, overdist=False):
    cmap = plt.cm.coolwarm
    cmap.set_bad('black')

    data = np.array(data)
    print('plot, params:',[( params[p].name, params[p].value) for p in params])
    model =  model_img(params, data.shape, psf_cube=psf_cube, spectrum_model=spectrum_model, overdist=overdist)
...
def subtract_psf(data,psf_cube,spectrum_model,params,mask_fitting,
                 algorithm = 'mcmc',subtract=False,save_results=True):
    ...
    if algorithm == 'mcmc':
        ...
        if 0:
            def fcn2min(params):
                m_i = model_img(params=params, cube_shape=data_tmp.shape, psf_cube=psf_cube, spectrum_model=spectrum_model, overdist=False,
        ...
        print('prepare emcee')
        vary_params = [p for p in params_new.values() if p.vary]
        par_names = [el.name for el in vary_params]
        ndim = len(vary_params)
        nwalkers, nsteps = 100, 100


        def lnprior(theta):
            for el, val in zip(vary_params, theta):
                el.value = val
            for p in params_new.values():
                ...
                if p.value > p.max:
                    return -np.inf
            xc = params_new['xc'].value
            yc = params_new['yc'].value
            if (0 <= int(xc) < mask_new.shape[0]
                    and 0 <= int(yc) < mask_new.shape[1]
                    and mask_new[int(xc), int(yc)]):
                return 0
            else:
                return -np.inf
        ...
        def log_probability(theta, data):
            ...
            chi_prior = lnprior(theta)
            if ~np.isinf(chi_prior):

                m_i = model_img(params_new, data_tmp.shape, psf_cube=psf_cube, spectrum_model=spectrum_model,
                                overdist=False,
                                debug=False, get_qso_pos=False)

                residuals = data_tmp - m_i
                residuals[:, ~mask_new] = np.nan
        ...
        #init mcmc
        init = [p.value for p in vary_params]
        init_range = [p.stderr if p.stderr is not None else 1e-3 for p in vary_params]
        p0 = []
        for i in range(nwalkers):
            ...
            while prob == -np.inf:
                ...
                prob = log_probability(theta=wal_pos,data=data_tmp)
            p0.append(wal_pos)
        p0 = np.array(p0)
        ...
        if 1:
            ...
            sampler = emcee.EnsembleSampler(nwalkers, ndim, log_probability, args=[data_tmp])
            sampler.run_mcmc(p0, nsteps, progress=True)
        else:
            #multiprocessing
            with Pool() as pool:
                sampler = emcee.EnsembleSampler(nwalkers, ndim, log_prob_fn=log_probability, args=[data_tmp], pool=pool)
                sampler.run_mcmc(p0, nsteps, progress=True)
    ...
    plot_comparison(data_new, params_new, psf_cube, mask_new, spectrum_model, overdist=False)
    ...
    plot_comparison(data, params, psf_cube, mask_fitting, spectrum_model, overdist=False)

    if save_results:
        model = model_img(params, data.shape, psf_cube=psf_cube, spectrum_model=spectrum_model, overdist=False)
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        for name, value in zip(names, [cenA[0], cenA[1], 1.0,0]):
            params.add(name, value=value, min=0, max=np.inf)
        params['psf_rot'].min=-90
        params['psf_rot'].max = 90
    if 0:
        #
        model_convolved = np.zeros_like(data)
        from scipy.ndimage import rotate

        psf_rot = rotate(
            psf,
            -10,
            axes=(1, 2),  # rotate spatial axes only
            reshape=False,
            order=3
        )
        m = model_img(params, cube_shape, psf_cube=psf_rot, spectrum_model=norm_flux_1sigma,overdist=False, debug=False, get_qso_pos=False)

        fig, ax = plt.subplots(1, 4, figsize=(16, 4),sharex=True, sharey=True)

        ax[0].imshow(np.log10(np.abs(image)),vmin=1,vmax=3, origin='lower')
        ax[0].set_title("Observed image")

        ax[1].imshow(np.log10(np.nanmean(psf, axis=0)), origin='lower')
        ax[1].set_title("PSF (mean over wavelength)")

        ax[2].imshow(np.log10(np.nanmean(psf_rot, axis=0)), origin='lower')
        ax[2].set_title("PSF (mean over wavelength)")

        ax[3].imshow(np.log10(np.abs(np.nanmean(m, axis=0))),vmin=1,vmax=3, origin='lower')
        ax[3].set_title("Model (unconvolved)")
        ...
        #cube.data = model_convolved

        # (optional but recommended) update history
        #cube.history.append("Replaced data with PSF-convolved model")

        # save new file
        #cube.save(path_to_jwst_folder + '/output/detector3/' +'model_convolved_cube_s3d.fits')
    ...
    if 1:
        ...
        model_A = np.zeros_like(data)
        model_B = np.zeros_like(data)
        maskA = rr <= miri_psf_sigma_pix
        maskB = rr_B <= miri_psf_sigma_pix
        fitting_radius = 2*miri_psf_sigma_pix #pix
        mask_radius = 3 * miri_psf_sigma_pix
        flux_1sigma_A = np.nansum((data - model_B)[:, maskA], axis=1)
        flux_1sigma_B = np.nansum((data - model_A)[:, maskB], axis=1)
        save_model = True
        ...
        for it in range(2):
            ...
            modelA, params_A = subtract_psf(data = data, psf_cube=psf, spectrum_model=flux_1sigma_A, params=params,
                         mask_fitting=mask_fitting,algorithm='mcmc', subtract=False)
            #save model
            if save_model:
                ...
                cube.history.append("Replaced data with PSF-convolved model")
                # save new file
                cube.save(path_to_jwst_folder + '/output/detector3/' + 'model_psf_cube_A_s3d.fits')
                # save psf subtracted cube
                cube.data = data - modelA
                # (optional but recommended) update history
                cube.history.append("Replaced data with PSF-subtracted data")
                # save new file
                cube.save(path_to_jwst_folder + '/output/detector3/' + 'psf_subtracted_cube_A_s3d.fits')
            ...
            modelB, params_B = subtract_psf(data=data, psf_cube=psf, spectrum_model=flux_1sigma_B, params=params,
                                          mask_fitting=mask_fitting, algorithm='mcmc', subtract=False)
            # save model
            if save_model:
                ...
                # save new file
                cube.save(path_to_jwst_folder + '/output/detector3/' + 'model_psf_cube_B_s3d.fits')
                cube.data = data - modelB
                # (optional but recommended) update history
                cube.add_history_entry("Replaced data with PSF-subtracted data")
                # save new file
                cube.save(path_to_jwst_folder + '/output/detector3/' + 'psf_subtracted_cube_B_s3d.fits')
